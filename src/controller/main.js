...
const regView = async (req, res) => {
    ...
    try {
        ...
        await pool.execute("INSERT INTO user (id, first_name,last_name,full_name) VALUES (?,?,?,?)", [ID,fname,lname,full_name])
        // ----------------------------------------------------------------
...
const logoutView = async (req, ers) => {
    return res.send("DONE logut")
}
export default { mainView, loginView, regView, logoutView }